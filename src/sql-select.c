...
static int callback(void *data, int argc, char **argv, char **azColName){
   ...
   if (argc >= 6) {
     printf("[{\"HostName\":\"%s\",\"%s\":%s,\"%s\":%s,\"%s\":%s,\"%s\":%s,\"%s\":%s,\"%s\":%s}]",
	 hostname, azColName[0], argv[0] ? argv[0] : "NULL", 
		   ...
		   azColName[5], argv[5] ? argv[5] : "NULL");
     printf("\n");
   }
#ifdef DEBUG
   for(i = 0; i<argc; i++) {
...
int main(int argc, char* argv[]) {
   ...
   sql = "SELECT app_state, switch_state, rhum_value, rhum_state, fan_state, i2cerror from Fan;";
